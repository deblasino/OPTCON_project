...
for kk in range(max_iters-1 ):
    ...
    for tt in reversed(range(TT-1)):
        ...
        #compute lambdas and dJu for each t, only for armijio because we don't need the hessian of dynamics because of regularization
        lambdas[:,tt] = A_t[:, :, tt].T @ lambdas[:,tt+1] +qt
        dJ[:,tt,kk] =  B_t[:, :, tt].T @ lambdas[:,tt+1] + rt


        #Compute matrixes to solve LQR problem to find the descending direction, using regularization techniques to not consider the hessian of dynamics
        Q_t = cst.QQt
        R_t = cst.RRt
        S_t = np.zeros((ni, ns))
